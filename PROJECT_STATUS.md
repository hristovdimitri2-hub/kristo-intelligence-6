...
**Тестове:** 317/317 (нови: retention идемпотентност, прагът по всички публични
повърхности, честната backfill аритметика; сверени стари очаквания за $50k в три тестови
файла). **Живи проверки (18.09):** 285 реда ≥$5M, **0 дубликати** (проверени групи по
`(tx_hash, log_index)`), таблица **208 kB**, дашборд `state=live_data`,
`threshold_usdc=5 000 000`, първите нови китове $40.4M и $5.7M; скенерът догонва
24-часовия прозорец и после става live.
