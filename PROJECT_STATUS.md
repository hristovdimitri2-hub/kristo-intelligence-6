...
- ⚠️ **Платилият не получава нищо насаме** — няма връзка wallet↔чат, затова поканата отива
  при собственика. Днес единственият канал е публичният, значи купувачът чака ръчна доставка.
  Опции: (а) `TELEGRAM_VIP_CHAT_ID` = частен чат на собственика (най-бързо); (б) купувачът
  пише на бота (личният чат вече работи и `/claim`-пътят е налице); (в) пълно обвързване
  wallet↔чат с подпис.
- ⚠️ `_vip_subscribers` / `_vip_invites` са в RAM → губят се при деплой (продажбата в
  Postgres остава).
