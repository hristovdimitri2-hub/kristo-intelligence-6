...
## 📌 ОТВОРЕНИ КОРЕНИ И ДЪЛГОВЕ (25.09) — само документация, без задачи

Четири неща, известни и записани, подредени по спешност. Нищо от тук не е
спешно; нищо от тук не е забравено.

1. **Placeholder hash при claim.** Claim пътят **може да запише
   `block_hash='000…0'` (вместо NULL) при node-lag прозорец** — случи се на
   23.09 (tx `0x22c4…52ab`, детайлите в секцията за loop-а по-долу).
   Детекторът е **имунизиран** (фикс `3266a9f` — всички-нули = „няма anchor
   данни"). **Коренът — „записвай NULL при липса на hash", никога `'000…0'`
   (write-странична защита в receipt-блока на claim-пътя)** — кандидат за
   **следващата инженерна вълна, не спешно**.
   **ДОПЪЛНЕНИЕ (25.09) — уточнение на бъдещата задача (замества общото
   формулиране по-горе):** задачата е **конкретна, не общ преглед** —
   открий **откъде хешът остава празен точно в node-lag клона**
   (пътят на `waited_and_accepted` — receipt-блокът на claim-пътя в
   `main.py`, където receipt-ът се чете преди `claim_payment_tx`;
   **3-те случки** = 3× `waited_and_accepted` в живите `guard_events`)
   и **подай NULL на това конкретно място, никога placeholder `'000…0'`**.
   Тригерът е известен (node-lag прозорецът ⇒ липсващ hash ⇒ конкретната
   точка в клона) — спестява диагностична обиколка. Остава кандидат за
   **следващата инженерна вълна, не спешно**; дотогава детектор-защитата
   (`3266a9f`) е достатъчна.
2. **Flaky тест:** `tests/test_whaleflow.py::test_whale_scan_halves_a_refused_window_instead_of_giving_up`
   — timing-dependent: минава сам и в целия си файл, но веднъж от 4 пълни
   минавания (25.09) се провали в suite контекста (`max(attempts)==250` vs
   5000 — интерференция от външен state, не от кода). **Поглед при следваща
   сесия.** Собственият ни текущ изглед е **412/412**.
3. **F2b (от одит №4, 22.09):** transaction-shape payload-ите (без EIP-3009
   nonce) — техният идемпотентен ключ е **tx hash** (детерминиран от raw
   bytes), заключен на hash, вместо на authorization nonce. **Дефинирано,
   чака вълна.**
4. **TODO тест:** retry след **>2.8h (извън 5000-блоковия прозорец)** —
   архитектурата е безопасна (fail-closed; непотвърденият revert никога не
   обещава 425), **тестът липсва**. Добавя се при следващото пипане на
   прозореца/адаптивния window.
...
**⚠️ ОТВОРЕН КОРЕН (запис за бъдещето — БЕЗ задача сега):** claim пътят
**може да запише placeholder `block_hash='000…0'` при node-lag прозорец**
(точно това се случи на 23.09 — редът е написан в лаг-прозореца, не от
миграцията). Детекторът вече е **имунизиран** (условието по-горе, `3266a9f`);
**корен-фиксът — при липса на hash да се записва NULL/`''`, НИКОГА `'000…0'`
(write-странична защита в claim-пътя, `main.py` receipt-блокът)** — кандидат
за **следващата инженерна вълна, не спешно**. Дотогава детектор-защитата е
достатъчна: шумът не може да се върне, а сигналът (реален reorg) остава жив.
...
**линкът е бил готов от 18.09 + price snapshot корекция (0.003–0.005). Нишката:**
**чакаме реакция по snapshot-а.**
