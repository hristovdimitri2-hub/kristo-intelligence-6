...
## 📡 RANKING ПУЛС 1 — механизъм установен, чекпойнтите анулирани (20.09)

**RANKING ПУЛС 1 (20.09, ~30 мин след откриване на часовниковата корекция):** **ABSENT** за
eth/kaito/ondo/degen; **#1 defi**, **#2 signals / whale / rug**.

**МЕХАНИЗЪМ УСТАНОВЕН:** PayAPI класира по **листинг-описанието** (старо, от 06.09), **НЕ по
openapi**. Индексираният текст е: „Live Base DeFi signals, whale USDC tracking, cross-DEX arb
spreads & rug checks. x402-native micro-payments, no signup needed." → четирите ни класации
идват от четирите думи, които са в него, а четирите липсващи са думите, които не са там.
Търсене: `match_mode: all_tokens`, `verified_only: true`, `searched: 170`.

**Чекпойнтите 24/72ч са АНУЛИРАНИ** — безполезни за този канал. (Часовникът ZERO беше
2026-09-20 20:42:42 UTC, deploy `dep-dao4agnf3r…`, commit `76fa746` — остава само като факт за
последния успешен деплой, не като тригер; добавянето на тикери в `openapi.json` (15.09,
`01e8ff1`) не може да мръдне тази класация.)

**Единствените пътища:** (1) смяна на **листинг-описанието** — **Текст 1, изпратен към Chet
20.09**; или (2) **reindex** — въпросът е в същото писмо. **Отговорът на Chet решава кое.**

**whale #2 (не #1):** нов верифициран конкурент с „whale" в името — записано.
...
**Render факти (за проследимост):** deploy `dep-dao4agnf3r…`, статус `live`,
created `2026-09-20T20:42:42Z`, finished `2026-09-20T20:43:37Z`, commit
`76fa746b1985b7bea7ef87d954d7a0f89df1c84c` (в бележката на собственика стоеше `76fa7a46` —
същият комит).
