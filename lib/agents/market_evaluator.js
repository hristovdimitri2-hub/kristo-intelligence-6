...
/**
 * Escape a value for Telegram's legacy parse_mode:'Markdown'.
 *
 * Audit #6 BREAK 3: entity toggles (* _ ` [) pair across the WHOLE message,
 * so the DeFiLlama branch's literals — category 'defi_protocol' (1 '_') +
 * type 'high_tvl_protocol' (2 '_') = 3 — left an UNPAIRED underscore and
 * Telegram rejected EVERY such notification with 400
 * "can't parse entities". Every interpolated value is escaped so no raw
 * toggle can ever enter the message; backslash escapes are honored by
 * Telegram's parser (per their formatting docs).
 */
...
/**
 * Build the approval-notification text. Pure and exported so the test
 * suite can run REAL trends (and the 10 live DeFiLlama names that broke
 * in audit #6) through the REAL builder and lint the result.
 */
function buildTrendApprovalText(trend) {
  return [
    `🔔 *Нов пазарен тренд открит!*`,
    ``,
    `*Име:* ${mdEsc(trend.title)}`,
    `*Категория:* ${mdEsc(trend.category)}`,
    `*Източник:* ${mdEsc(trend.source)}`,
    `*Demand Score:* ${Number(trend.demandScore) || 0}/100`,
    `*Тип:* ${mdEsc(trend.type)}`,
    ``,
    `*Описание:*`,
    `${mdEsc(trend.description)}`,
    ``,
    `💡 Предложена x402 микро-услуга: \`${mdEsc(trend.suggestedService)}\``,
    ``,
...
async function sendTrendApprovalNotification(trend) {
  ...
  const text = buildTrendApprovalText(trend);
 ...
 *   - Пълният сигнал остава зад x402 paywall (от 0.003 USDC на заявка, per route).
...
function generateFarcasterTeaser(trend) {
  ...
  const cta = `→ Пълен сигнал + on-chain анализ: платена x402 версия (от 0.003 USDC на заявка)`;
...
// Entry guard: run only when invoked directly (`node lib/agents/...`), so
// requiring the module (tests, tooling) never starts scan loops.
...
// Exported for the test suite (audit #6): the REAL builder under test.
module.exports = { mdEsc, buildTrendApprovalText };